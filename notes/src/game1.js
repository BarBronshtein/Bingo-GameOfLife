...
var BOARD_SIZE = 5 // Note: Board size by size
var gNums = resetNums()
var btn = document.querySelector('.btn-bingo')
...
var gPlayers = [
  {
    ...
    // elMatrixPlayer is the better name
...
// I'd like to see an init function, that initilizes your variables for you, as aswell as
// add event listeners, like this one

btn.addEventListener('click', playBingo)
...
function playBingo() {
  ...
  gGameInterval = setInterval(playTurn, 200)
}
function playTurn() {
  gIsRunning = true
  var isVictory = false
  ...
  if (isVictory) {
    console.log(
      `Congratulations🎆🎆🎆 ${gPlayers[i - 1].name} you have won the game`
    )
...
function displayOnScreen(players) {
  for (let i = 0; i < players.length; i++) {
    players[i].boardOnScreen = createBoardOnScreen(players[i])
    players[i].matrixPlayerEl.classList.remove('hidden')
  }
}

function printBingoBoard(player, board) {
  let visualBoard = []
  let html = ''
  for (let j = 0; j < BOARD_SIZE; j++) {
    ...
    for (let i = 0; i < BOARD_SIZE; i++) {
      ...
      if (board[j][i].isMainDiagonal) board[i][i] = '💘'
      visualBoard[j][i] = board[j][i].value
      html += `<td>${visualBoard[j][i]}</td>`
    }
    html += `</tr>`
  }
...
function createBoardOnScreen(player) {
  let html = ''
  for (let i = 0; i < BOARD_SIZE; i++) {
    html += `<tr>`
...
function markBoard(player, calledNum) {
  ...
  if (curHitCount === player.hitCount) return false
    ...
    printBingoBoard(player, player.board)
    return true
  }
}
